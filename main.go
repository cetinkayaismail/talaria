...
func main() {
	...
	// --- SERVICES MODULE ---
	if (runAll || selectedModules["services"]) && !excludedModules["services"] {
		wg.Add(1)
		go func() {
			defer wg.Done()
	...
	// --- PACKAGES MODULE ---
	if (runAll || selectedModules["packages"]) && !excludedModules["packages"] {
		wg.Add(1)
		go func() {
			defer wg.Done()
...
func saveReport(report *models.ScanReport, path string, format string, encryptKey string) {
	...
	if strings.ToLower(format) == "json" {
		...
		// --- Helper to write a generic dangerous-only section ---
		...
		sb.WriteString(fmt.Sprintf("\nSummary Counts:\nSecrets: %d | SUID: %d | SGID: %d | Sudo: %d | Sudo Tokens: %d\nCapabilities: %d | CronJobs: %d | Timers: %d | Writeable: %d\nNetwork: %d | NFS: %d | Processes: %d | Sockets: %d\nFilePerms: %d | FilePermsExploit: %d | Groups: %d | PATH Hijack: %d\nSSH Keys: %d | Container: %d | D-Bus: %d | Services: %d | Packages: %d\nVulnerabilities: %d | Sessions: %d | KernelConfig: %d | Polkit: %d\nHistory: %d | XAuth: %d | Logrotate: %d | EnvFile: %d\nPAM: %d | Sysctl: %d | SystemdOverrides: %d | SubUID: %d\nMounts: %d | ELF RPATH: %d | Auditd: %d | Udev: %d\nCronDirs: %d | ProcEnv: %d | LD/NSS: %d | Modprobe: %d\nCloudMeta: %d | VenvWrap: %d | SudoTokens: %d\n",
			...
			len(report.CloudMetaResults), len(report.VenvWrapResults), len(report.SudoTokens)))
		data = []byte(sb.String())
	}
